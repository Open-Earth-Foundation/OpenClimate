...
async function cleanup() {
    ...
    await EmissionsAgg.destroy({where: {datasource_id: emissionsDataSource1Props.datasource_id}})
    await EmissionsAgg.destroy({where: {datasource_id: emissionsDataSource2Props.datasource_id}})

    await Actor.destroy({where: {datasource_id: geoDataSourceProps.datasource_id}})

    await DataSource.destroy({where: {datasource_id: geoDataSourceProps.datasource_id}})
    await DataSource.destroy({where: {datasource_id: emissionsDataSource1Props.datasource_id}})
    await DataSource.destroy({where: {datasource_id: emissionsDataSource2Props.datasource_id}})
...
it("can create and get EmissionsAgg", async () => {
    ...
    let match = await EmissionsAgg.findOne({where: {
    ...
    expect(match.total_emissions).toEqual(String(countryDataSource1Year1Props.total_emissions))

    // Match on actor_id

    let matches = await EmissionsAgg.findAll({where: {
        actor_id: regionProps.actor_id
    }})
