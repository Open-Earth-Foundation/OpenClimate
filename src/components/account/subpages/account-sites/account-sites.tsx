...
const AccountSites: FunctionComponent<IProps> = (props) => {
    ...
    const updateAggregateEmissions = (dsSites: Array<ISite>) => {
        const filteredAggrEmissions = aggregatedEmissions?.filter(ae => dsSites?.some(s => s.facility_name == ae.facility_name));
        if(filteredAggrEmissions)
        {
            const summaryAggrEmissions = AggregatedEmissionHelper.GetSummaryAggregatedEmissions(filteredAggrEmissions);
