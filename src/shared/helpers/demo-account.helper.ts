...
async function demoOrganizationExists () {
    ...
    {
        const demoUser = await organizationService.getByCredentialId(DemoData.DemoOrganization.organization_credential_id);
        
        if(Object.keys(demoUser).length)
            return true;    
        else
            return false;
    }
...
async function SaveDemoDataToDb() {
    ...
    if(DemoData.DemoOrganization.organization_credential_id)
    {
        const org = await organizationService.getByCredentialId(DemoData.DemoOrganization.organization_credential_id);
