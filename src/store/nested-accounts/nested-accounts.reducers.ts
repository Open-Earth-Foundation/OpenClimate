...
export const nestedAccountsReducer = ( state = initialState, action: any ) => {
    switch ( action.type ) {
        ...
        case nestedAccountsTypes.LOAD_GEO_SUBNATIONAL:
            return {
                ...state,
                geoSubnationals: [...state.geoSubnationals, action.payload.geoSubnational],
            };
        case nestedAccountsTypes.LOAD_CLIMATE_ACTIONS: 
            return {
                ...state,
                loadedClimateActions: state.loadedClimateActions.concat(action.payload.climateActions)
            };
        case nestedAccountsTypes.NESTED_ACCOUNTS_CLEAR_STATE: return initialState;
