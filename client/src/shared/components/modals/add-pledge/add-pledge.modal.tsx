...
import { PledgeTargetType } from '../../../../api/models/shared/pledge/pledge-target-types';
...
const AddPledgeModal: FunctionComponent<Props> = (props) => {
    ...
    useEffect(() => {
        if(!pledgeSchemas.length) {
            ...
            schemaService.getSchemasByCategory("Pledges").then(response => {
                return response.json();

            }).then((response:any) => {
                const schemasAsArray = Object.keys(response).map((key) => response[key]);
                setPledgeSchemas(schemasAsArray);

                const options = schemasAsArray?.map((rf:any) => {
                    return {
...
/*
 {fieldsHtml.length ? 
                    <div className="test">{dynamicFormPart}</div>
                : ""
                    }
