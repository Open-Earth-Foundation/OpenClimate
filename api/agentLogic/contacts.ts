  ...
  try {
    ...
    if (connectionMessage.state === "request") {
      ...
      if (connectionMessage.state === "active") {
        ...
        await Websockets.sendMessageToAll(
          "WALLET",
          "WALLET_CONNECTION_SUCCESS",
          { did: connectionMessage.their_did }
        );
        ...
        await Websockets.sendMessageToAll("WALLET", "WALLET_PROOF_SENT", {
          did: connectionMessage.their_did,
        });
      }
    }
      ...
      if (connection.user_id && !connection.business_wallet) {
        ...
        if (user) {
          ...
          attributes = [
            ...
            {
              name: "access_role",
              value: user.Roles[0].dataValues.role_name
                ? user.Roles[0].dataValues.role_name
                : "",
            },
            {
              name: "user_name",
