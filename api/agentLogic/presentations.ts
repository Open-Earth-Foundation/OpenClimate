    ...
    if (cred_issuer) {
      ...
      if (AnonWebsockets.checkWebsocketID(message.connection_id)) {
        ...
        if (isScope1Creds(message.presentation.requested_proof.revealed_attrs)) {
          ...
          Scope1Creds.credential_issuer_name = { "sub_proof_index": 0, "raw": cred_issuer.organization_name, "encoded": cred_issuer.organization_name };
          Scope1Creds.cred_def_id = { "sub_proof_index": 0, "raw": message.presentation.identifiers[0].cred_def_id, "encoded": message.presentation.identifiers[0].cred_def_id };
          await Websockets.sendMessageToAll('EMISSIONS', 'RECEIVED', {facility_emissions_scope1_co2e: Scope1Creds})
        }

        if (isOrgVerificationCreds(message.presentation.requested_proof.revealed_attrs)) {
          const connection = await Connections.readConnection(message.connection_id)
          const user = await Users.readUser(connection.user_id)
          const wallet = await Wallets.addWallet(connection.their_did, user.user_id)
          await Websockets.sendMessageToAll('WALLET', 'ACCEPTED', {wallet: wallet})
        }
      }
    } else {
