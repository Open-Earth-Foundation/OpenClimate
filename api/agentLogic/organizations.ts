...
const createOrganization = async function (
  ...
  try {
    ...
    if (duplicateOrganization)
      return {error: 'ORGANIZATION ERROR: An organization with this name already exists.'}

    const organization = await Organizations.createOrganization(name, category, type, country, jurisdiction)

    // Broadcast the message to all connections
    Websockets.sendMessageToAll('ORGANIZATIONS', 'ORGANIZATIONS', {organizations: [organization]})
