...
const UNFCCC_ANNEX_1_CSV = path.join(
  __dirname,
  "..",
  "upstream-data/unfccc-annex-1/historical_emissions.csv"
);
...
exports.down = function (db) {
  return db
    .runSql(
      `DELETE from emissions_to_countries WHERE emissions_id in (SELECT emissions_id FROM emissions WHERE data_provider_id = ${UNFCCC_DATA_PROVIDER_ID})`
    )
    .then(() => {
      return db.runSql(
        `DELETE FROM emissions WHERE data_provider_id = ${UNFCCC_DATA_PROVIDER_ID}`
      );
    })
    .then(() => {
      return db.runSql(
        `DELETE FROM dataproviders where data_provider_id = ${UNFCCC_DATA_PROVIDER_ID}`
      );
    });
};
