    ...
    passport.authenticate("local", (err, user, info) => {
      if (err) {
        ...
        req.logIn(user, async (err) => {
          ...
          const connection = await Connections.readConnectionByUserId(
            req.user.user_id
          );
          let connection_id = "";
          ...
          let company = await Organizations.getOrganization(
            user.organization_id
          );
          if (
            user &&
            user.dataValues &&
            user.dataValues.Organization &&
            user.dataValues.Organization.dataValues
          ) {
    ...
    try {
      if (req.body) {
        ...
        if (!userByEmail || userByEmail === undefined) {
          ...
          // Put roles in the array
          const userRoles = [];
          userByEmail.Roles.forEach((element) =>
            userRoles.push(element.role_name)
          );
          ...
          if (
            ...
            userByEmail.dataValues.Organization &&
            userByEmail.dataValues.Organization.dataValues
          )
            company = userByEmail.dataValues.Organization.dataValues;
