...
it("can create and get EmissionsAgg", async () => {
    ...
    let match = await EmissionsAggTag.findOne({where: {
        tag_id: tag1.tag_id,
    ...
    let matches = await EmissionsAggTag.findAll({where: {tag_id: tag1.tag_id}})

    expect(matches.length).toEqual(2)

    matches = await EmissionsAggTag.findAll({where: {emissions_id: countryEmissions1Props.emissions_id}})
    ...
    await EmissionsAggTag.destroy({where: {emissions_id: countryEmissions1Props.emissions_id}})

    matches = await EmissionsAggTag.findAll({where: {emissions_id: countryEmissions1Props.emissions_id}})

    expect(matches.length).toEqual(0)

    matches = await EmissionsAggTag.findAll({where: {emissions_id: countryEmissions2Props.emissions_id}})

    expect(matches.length).toEqual(2)

    await EmissionsAggTag.destroy({where: {emissions_id: countryEmissions2Props.emissions_id}})
