...
it('can create, read, update and delete a DataSourceTag', async () => {
    ...
    let match = await DataSourceTag.findOne({where: {tag_id: tag1.tag_id, datasource_id: datasource1.datasource_id}})
    ...
    matches = await DataSourceTag.findAll({where: {datasource_id: datasource1.datasource_id}})
    ...
    await DataSourceTag.destroy({where: {datasource_id: datasource1.datasource_id}})

    matches = await DataSourceTag.findAll({where: {datasource_id: datasource1.datasource_id}})

    expect(matches.length).toEqual(0)

    matches = await DataSourceTag.findAll({where: {datasource_id: datasource2.datasource_id}})

    expect(matches.length).toEqual(2)

    await DataSourceTag.destroy({where: {datasource_id: datasource2.datasource_id}})
