...
const cleanup = async () => {
    ...
    await Promise.all([
        ActorDataCoverage.destroy({where: {actor_id: countryCoverageProps.actor_id}}),
        ActorDataCoverage.destroy({where: {actor_id: regionCoverageProps.actor_id}}),
        ActorDataCoverage.destroy({where: {actor_id: cityCoverageProps.actor_id}})
    ])
...
it("can create and get ActorDataCoverage data", async () => {
    ...
    let match = await ActorDataCoverage.findOne({where: {
        actor_id: countryProps.actor_id
    }})
    ...
    let matches = await ActorDataCoverage.findAll({where: {
        actor_id: countryProps.actor_id
    }})
