...
it("can create and get target tags", async () => {
  for (let target_id of [
    countryTarget1Props.target_id,
    regionTarget1Props.target_id,
  ]) {
  ...
  let match = await TargetTag.findOne({
    where: { target_id: countryTarget1Props.target_id, tag_id: tag1.tag_id },
  });
  ...
  let matches = await TargetTag.findAll({
    where: {
  ...
  matches = await TargetTag.findAll({
    where: {
      tag_id: tag2.tag_id,
    },
  });
  ...
  await TargetTag.destroy({
    where: { target_id: countryTarget1Props.target_id },
  });
  ...
  await TargetTag.destroy({
    where: { target_id: regionTarget1Props.target_id },
  });
