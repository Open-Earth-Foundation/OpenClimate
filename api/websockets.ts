        ...
        switch (type) {
          case "GET_ALL":
            if (check(rules, userCookieParsed, "organizations:read")) {
              ...
              sendMessage(ws, "ORGANIZATIONS", "ORGANIZATIONS", {
                organizations,
              });
            } else {
          ...
          case "CREATE":
            if (check(rules, userCookieParsed, "organizations:create")) {
              ...
              if (newOrganization.error) {
                logger.error({
                  name: newOrganization.error.name,
                  message: newOrganization.error.message,
                });
                sendMessage(
                  ws,
                  "ORGANIZATIONS",
          ...
          case "UPDATE":
            if (check(rules, userCookieParsed, "organizations:update")) {
              logger.debug(data);
        ...
        switch (type) {
          case "CREATE_SINGLE_USE":
            if (check(rules, userCookieParsed, "invitations:create")) {
              ...
              if (data.workflow) {
                invitation =
                  await Invitations.createPersistentSingleUseInvitation(
                    data.workflow
                  );
              } else {
            ...
            try {
              ...
              if (conn.connection_id) {
                ...
                AdminAPI.Presentations.requestPresentation(
                  conn.connection_id,
                  [
                    "facility_emissions_scope1_co2e",
