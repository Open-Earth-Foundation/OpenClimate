...
router.post('/api/:orgId/aggregated-emission', wrap(async (req: any, res: any) => {
    ...
    const foundEmission = await AggrEmissions.findOne({where:
        { 'data.facility_name': aggregatedEmissionData['facility_name'] }
    });
...
router.get('/api/:orgId/aggregated-emission/all', wrap(async (req: any, res: any) => {
    req.logger.debug(req.params.orgId);
    const allAggregatedEmissionsByOrg = await readAggrEmissionsByOrgId(req.params.orgId);
...
router.get('/api/aggregated-emission/all', wrap(async (req: any, res: any) => {
