...
import {createSite, readSitesByOrgId, readSites, findAllByCountry, findAllByCountryAndJur} from '../orm/sites';
...
router.get('/api/site/byJurisdiction/:countryName/:jurisdiction', async (req: any, res: any) => {
    try {
        const allSitesByJurisdiction = await findAllByCountryAndJur(req.params.countryName, req.params.jurisdiction);
        const sites = [];
