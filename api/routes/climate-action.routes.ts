...
router.post('/api/:orgId/climate-action', async (req: any, res: any) => {
    try {
        const addedClimateAction = await Climactions.createClimaction(req.params.orgId, req.body)   
...
router.get('/api/:orgId/climate-action/all', async (req: any, res: any) => {
    try {
        ...
        const allClimateActionsByOrg =  await Climactions.readClimactionsByOrgId(req.params.orgId)
...
router.get('/api/climate-action/:siteName', async (req: any, res: any) => {
    try {
        ...
        const climateActionsBySite = await Climactions.readClimactionsByFacilityName(req.params.siteName, req.params.orgId)
        const climateActions = [];
