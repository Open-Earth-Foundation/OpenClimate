...
import {addTrustedDID, readTrustedDID, readTrustedDIDByOrgName, readTrustedDIDs, checkTrustedDID} from '../orm/trusted_registry';

const router = Router()

router.post('/api/:trustedDID/add', async (req: any, res: any) => {
    try {
        const addedDID = await addTrustedDID(req.params.trustedDID, req.body.organization_name)
