...
it("gets a 404 on path to non-existent actor", async () =>
  request(app).get(`/api/v1/actor/${ACTOR_DNE}/path`).expect(404));
...
it("can get actor emissions", async () =>
  request(app)
...
it("can download actor emissions in json", async () =>
  request(app)
    .get(`/api/v1/download/${country1Props.actor_id}-emissions.json`)
    .expect(200)
    .expect("Content-Type", /json/)
