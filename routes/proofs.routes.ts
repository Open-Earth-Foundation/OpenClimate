...
router.post('/api/proof/:userID/:credID/add', async (req: any, res: any) => {
    try {
        ...
        const addedProof = await Proofs.createProof(req.params.userID, req.params.credID)
...
router.get('/api/proof/:userID/:credID/get', async (req: any, res: any) => {
    try {
        req.logger.debug("Request body", req.body)
        const proof = await Proofs.readProofsByCredDef(req.params.userID, req.params.credID)
