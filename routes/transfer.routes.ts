...
router.post('/api/:orgId/transfer', async (req: any, res: any) => {
    try {
        const addedTransfer = await Transfers.createTransfer(req.params.orgId, req.body);
...
router.get('/api/:orgId/transfer/all', async (req: any, res: any) => {
    try {
        ...
        const allTransfersByOrg = await Transfers.readTransfersByOrgId(req.params.orgId);
