...
router.get('/api/geodata/subnational/:countryCode/:subnationalName', wrap(async (req: any, res: any) => {
    const geoUrlResponse = await axios.get(`https://www.geoboundaries.org/api/current/gbOpen/${req.params.countryCode}/ADM1/`);

    const geoDataResponse = await axios.get(geoUrlResponse.data.gjDownloadURL);
    const features = geoDataResponse.data.features;
    const foundNode = features.filter(ft => ft.properties.shapeName.toLowerCase() == req.params.subnationalName.toLowerCase());
...
router.get('/api/geodata/country/:countryCode', wrap(async (req: any, res: any) => {
    ...
    let foundGeoData = await Geosubnationals.findAll({where:
    ...
    if(foundGeoData.length) {
        ...
        const geoUrlResponse = await axios.get(`https://www.geoboundaries.org/api/current/gbOpen/${countryCode}/ADM1/`);
        const geoDataResponse = await axios.get(geoUrlResponse.data.simplifiedGeometryGeoJSON);
