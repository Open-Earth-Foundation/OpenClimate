...
exports.up = async function(db) {
  ...
  if (!results.fields.find((f) => f.name == "URL")) {
    await db.addColumn('Target', 'URL', {type: 'string', length: 255, unique: false, notNull: false})
  }

  if (!results.fields.find((f) => f.name == "summary")) {
    await db.addColumn('Target', 'summary', {type: 'string', length: 255, unique: false, notNull: false})
  }

  await db.createTable(
    "TargetTag",
    {
      "target_id": {type: 'string', length: 255, unique: false, notNull: true, primaryKey: true},
      "tag_id": {type: 'string', length: 255, unique: false, notNull: true, primaryKey: true},
