...
const adminMessage = async (credentialIssuanceMessage) => {
  try {
    ...
    if (credentialIssuanceMessage.state === 'credential_issued' && credentialIssuanceMessage.schema_id === process.env.SCHEMA_VALIDATED_EMAIL) {
      if (AnonWebsockets.checkWebsocketID(credentialIssuanceMessage.connection_id)) {
