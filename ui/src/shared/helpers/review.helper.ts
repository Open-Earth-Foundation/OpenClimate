...
async function LoadEmissionsCountry(
  countryCode: string,
  ...
  const emissionObject: Record<string, { data: any; publisher: string }> =
    jsonData.data.emissions;
  ...
  sources.map((source) => {
    ...
    latestEmissions.map(
      (emissionData: { total_emissions: string; year: number }) => {
    ...
    const emissionInfo = {
      latestTotalEmissions:
        latestEmissions[latestEmissions.length - 1].total_emissions,
...
async function LoadEmissionsSubnational(
...
async function LoadEmissionsCity(city_id: number, entity: ITrackedEntity) {
  ...
  if (!emissionResponse) {
    ...
    const netEmission = 0;
    const sinks = grossEmission - 0;
    const date_updated = 0;
    const methodologies = [];
    const emissionData: IAggregatedEmission = {
      facility_ghg_total_gross_co2e: grossEmission,
      facility_ghg_total_sinks_co2e: sinks,
      facility_ghg_total_net_co2e: netEmission,
      facility_ghg_date_updated: "2022/10/10",
      facility_ghg_year: year,
      facility_ghg_methodologies: methodologies,
    };
...
async function LoadPledgesSubnational(organization: string) {
  ...
  const pledgesResponse = await pledgeService.fetchPledgesSubnational(
    organization
  );
  if (pledgesResponse && pledgesResponse.length) {
    ...
    for (let i = 0; i < rowNumbers.length; i++) {
      const baseYear = pledgesResponse.find(
        (d: any) =>
          d["column_name"] === "Base year" && d["row_number"] == rowNumbers[i]
      )?.response_answer;
      const targetYear = pledgesResponse.find(
        (d: any) =>
          d["column_name"] === "Target year" && d["row_number"] == rowNumbers[i]
      )?.response_answer;
      const reduction = pledgesResponse.find(
        (d: any) =>
          d["column_name"] === reductionStr && d["row_number"] == rowNumbers[i]
      )?.response_answer;
...
async function LoadPledgesCountry(country: string) {
  ...
  const pledgesResponse = await climateWatchService.fetchPledgesCountry(
    country
  );

  if (pledgesResponse && pledgesResponse.data && pledgesResponse.data.length) {
    const baseYear = pledgesResponse.data.find(
      (d: any) => d["indicator_id"] === "pledge_base_year"
    )?.value;
    const targetYear = pledgesResponse.data.find(
      (d: any) => d["indicator_id"] === "pledge_target_year"
    )?.value;
    const reduction = pledgesResponse.data.find(
      (d: any) => d["indicator_id"] === "M_TarA2"
    )?.value;
...
async function LoadTreatiesCountry(country: string) {
  const treatiesResponse = await climateWatchService.fetchTreatiesCountry(
    country
  );

  if (
    treatiesResponse &&
    treatiesResponse.data &&
    treatiesResponse.data.length
  ) {
    const submissionDateStr = "Latest submission date";
    const submissionDate = treatiesResponse.data.find(
      (d: any) => d["indicator_name"] === submissionDateStr
    )?.value;
    const signed = !!treatiesResponse.data.find(
      (d: any) => d["indicator_name"] === "Signed"
    )?.value;
    const ratified = !!treatiesResponse.data.find(
      (d: any) => d["indicator_name"] === "Ratified"
    )?.value;
...
async function GetTrackedEntity(
  ...
  switch (type) {
    case FilterTypes.National:
      await ReviewHelper.LoadEmissionsCountry(
        trackedEntity?.id.toString() ?? "",
        trackedEntity
      );
      ...
      trackedEntity.transfers = await transferService.allTransfersByCountry(
        option.name
      );
    ...
    case FilterTypes.SubNational:
      ...
      trackedEntity.flagCode = option.flag;
      trackedEntity.cities = await CountryCodesHelper.GetCitiesBySubnationalId(
        option.value
      );
      ...
      if (previousSelectedEntity) {
        if (previousSelectedEntity.countryCode)
          await ReviewHelper.LoadTreatiesCountry(
            previousSelectedEntity.countryCode,
            trackedEntity
          );
        if (previousSelectedEntity.countryName) {
          trackedEntity.sites = await siteService.allSitesByJurisdtiction(
            previousSelectedEntity.countryName,
            option.name
          );
          sites = trackedEntity.sites;
          trackedEntity.transfers =
            await transferService.allTransfersByJurisdiction(
              previousSelectedEntity.countryName,
              option.name
            );
        }
    ...
    case FilterTypes.Organization:
      ...
      const sitesByLocation = orgSites.filter(
        (s) =>
          s.facility_country === country &&
          s.facility_jurisdiction === jurisdiction
      );
      ...
      const aggrEmissions =
        await aggregatedEmissionService.allAggregatedEmissionsByOrg(
          option.value
        );
      const aggrEmissionsByLocation = aggrEmissions.filter((ae) =>
        siteNames.includes(ae.facility_name)
      );

      trackedEntity.aggregatedEmission =
        AggregatedEmissionHelper.GetSummaryAggregatedEmissions(
          aggrEmissionsByLocation
        );

      const pledges = await pledgeService.allPledges(option.value);
      trackedEntity.pledges = pledges
        .slice(Math.max(pledges.length - 3, 0))
        .reverse();
      let climateActions = await climateActionService.allClimateAction(
        option.value
      );

      const scope1EmsActions = ClimateActionHelper.GetClimateActions(
        climateActions,
        ClimateActionScopes.Scope1,
        ClimateActionTypes.Emissions
      );
      const scope1EmsTotal = ClimateActionHelper.GetSumC02(
        scope1EmsActions,
        "facility_emissions_co2e"
      );

      const scope2EmsActions = ClimateActionHelper.GetClimateActions(
        climateActions,
        ClimateActionScopes.Scope2,
        ClimateActionTypes.Emissions
      );
      const scope2EmsTotal = ClimateActionHelper.GetSumC02(
        scope2EmsActions,
        "facility_emissions_co2e"
      );

      const scope1MtsActions = ClimateActionHelper.GetClimateActions(
        climateActions,
        ClimateActionScopes.Scope1,
        ClimateActionTypes.Mitigations
      );
      const scope1MtsTotal = ClimateActionHelper.GetSumC02(
        scope1MtsActions,
        "facility_mitigations_co2e"
      );

      const scope2MtsActions = ClimateActionHelper.GetClimateActions(
        climateActions,
        ClimateActionScopes.Scope2,
        ClimateActionTypes.Mitigations
      );
      const scope2MtsTotal = ClimateActionHelper.GetSumC02(
        scope2MtsActions,
        "facility_mitigations_co2e"
      );
      ...
      trackedEntity.transfers = orgTransfers.filter(
        (t) =>
          (t.transfer_receiver_country === country &&
            t.transfer_receiver_jurisdiction === jurisdiction) ||
          (t.facility_country === country &&
            t.facility_jurisdiction === jurisdiction)
      );

      if (previousSelectedEntity) {
        trackedEntity.jurisdictionName =
          previousSelectedEntity.jurisdictionName;
        trackedEntity.jurisdictionCode =
          previousSelectedEntity.jurisdictionCode;
      }
...
const GetOrganizations = async (selectedEntity: ITrackedEntity) => {
  ...
  selectedEntity.sites
    ?.filter((site) => site.organization_name && site.organization_id)
    .forEach((site: ISite) => {
      const item = {
      ...
      if (!added) dropdownItems.push(item);
    });
...
const GetOptions = async (
  ...
  switch (filterType) {
    ...
    case FilterTypes.EntityType:
      ...
      break;
    case FilterTypes.City:
      if (selectedEntity) options = await getCities(selectedEntity);
      break;
    case FilterTypes.Organization:
      if (selectedEntity) options = await GetOrganizations(selectedEntity);
      break;
  }
  return options;
};
...
const createProviderEmissionsData = (data: Array<any>) => {
  ...
  data.forEach((emission) => {
    ...
    if (dataProviderName && !providerToEmissions[dataProviderName]) {
      const methodologyTags = emission.DataProvider.Methodology.Tags.map(
        (tag) => tag.tag_name
      );
      const emissionData: EmissionInfo = {
        ...
        methodologyType:
          emission.DataProvider?.Methodology?.methodology_type ?? "",
      ...
      if (
        !providerToEmissions[dataProviderName].totalGhg &&
        emission.total_ghg_co2e
      ) {
        providerToEmissions[dataProviderName].totalGhg =
          emission.total_ghg_co2e;
      }
      if (
        !providerToEmissions[dataProviderName].landSinks &&
        emission.land_sinks
      ) {
...
export const getChangedEmissionData = async (
  dataProviderId: number,
  entityType: FilterTypes | null | undefined,
  entity: ITrackedEntity | null | undefined
) => {
  switch (entityType) {
    case 0:
      ...
      filteredEmissions = filteredEmissions.filter(
        (f: any) =>
          f.DataProvider.data_provider_id === dataProviderId &&
          f.total_ghg_co2e !== null
      );
      ...
      if (filteredEmissions.length) {
        ...
        const netEmission = grossEmission - sinks;
        const date_updated =
          filteredEmissions[0].DataProvider.Methodology.date_update;
        const year = filteredEmissions[0].year;
        const methodologies =
          filteredEmissions[0].DataProvider.Methodology.Tags;
