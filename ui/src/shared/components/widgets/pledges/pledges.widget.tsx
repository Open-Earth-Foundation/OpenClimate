...
const PledgesWidget: FunctionComponent<Props> = (props) => {
  ...
  if (pledges?.length) {
    pledgeItems = pledges.map((p: IPledge, index: number) => {
      ...
      return <PledgeItem pledge={p} key={key} voluntary={voluntary} />;
    });
  ...
  const handlerAddPledge = () => {
    if (showModal) showModal("add-pledge");
  };
      ...
      <div className="widget__wrapper">
        <div className="widget__header">
          <div className="widget__title-wrapper">
            <h3 className="widget__title">Your pledges</h3>
            {detailsLink ? (
        ...
        <div className="widget__content">
          {pledges?.length ? (
            <>
              <div className="widget__pledges-content">{pledgeItems}</div>
              {showAddBtn ? (
            ...
            <div className="widget__no-data">
              <div className="widget__no-data-title">No data yet</div>
              {showAddBtn ? <AddNewBtn onClick={handlerAddPledge} /> : ""}
            </div>
          )}
        </div>
      </div>
