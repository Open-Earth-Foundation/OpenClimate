...
const ContextualDataWidget: FunctionComponent<Props> = (props) => {
  ...
  const latestPopulation =
    current && current.population && current.population.length > 0
      ...
      : null;
  const latestGDP =
    current && current.gdp && current.gdp.length > 0 ? current.gdp[0] : null;
  const parentPopulation =
    parent && parent.population && latestPopulation
      ? parent.population
      ...
      ? [
          {
            value:
              100 -
              Math.round(
                (latestPopulation.population / parentPopulation.population) *
                  100
              ),
            label: "Population",
            color: "#D9D9D9",
          ...
          {
            value: Math.round(
              (latestPopulation.population / parentPopulation.population) * 100
            ),
            label: "Population",
            color: "#2351DC",
          },
      ...
      {current.gdp && current.population && current.area ? (
        <div className="contextual-widget__wrapper ">
          <div className="contextual-widget__header">
            ...
            {lastUpdatedYear && (
              <span className="contextual-widget__updated">
                Last updated in {lastUpdatedYear}
              </span>
            )}
          </div>
          ...
          <div className="contextual-widget__wrapper">
            ...
            <div className="contextual-widget__data">
              <div className="contextual-widget__contextual-empty-state">
                <p>
                  There's no data available, if you have any suggested <br />{" "}
                  data sources or you are a provider please
                </p>
