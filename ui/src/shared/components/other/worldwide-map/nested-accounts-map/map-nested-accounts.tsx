...
const MapNestedAccounts: FunctionComponent<Props> = (props) => {
  ...
  const [showLoadSubnationals, setShowLoadSubnationals] =
  ...
  const [loadDefaultJurisdiction, setLoadDefaultjurisdiction] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    subnationalsGeoJson,
    subnationalsGeoJsonRef.current,
  ]);
  ...
  useEffect(() => {
    if (countrySelectedCode) {
      const found: any = geoSubnationals.find(
        (gs) => gs.name === countrySelectedCode
      );
  ...
  useEffect(() => {
    ...
    if (jurisdictionName) {
      ...
      if (countryCode) loadSubnationalsClickHandler(countryCode);
  ...
  const loadSubnationalsClickHandler = (countryCode: string) => {
    ...
    if (!foundGeoData) loadGeoSubnational(countryCode);
    else setSubnationalsGeoJson(foundGeoData.geoData);
  };
  ...
  const onEachFeatureSub = (feature: any, layer: any) => {
    ...
    if (
      (layer.feature.properties.shapeISO === defaultJurisdictionCode ||
  ...
  const siteClickHandler = (siteId: string) => {
    ...
    const splitedCoordinates = foundSite?.facility_bounds
      ?.split(";")
      .filter((c) => c);
  ...
  const onZoomChanged = (e: any) => {
    if (e.target._zoom > zoomDefault) setShowClearZoom(true);
    else setShowClearZoom(false);
  };

  const selectDefaultSite = () => {
    const siteDefaultId = sites?.find(
      (s) => s.facility_name === defaultSiteName
    )?.id;
    if (siteDefaultId) siteClickHandler(siteDefaultId);
  };

  const selectDefaultCountry = () => {
    const layerKey = Object.keys(countryGeoJsonRef.current._layers).find(
      (k: any) =>
        countryGeoJsonRef.current._layers[k].feature.properties.ISO_A3 ===
        defaultCountry
    );
  ...
  const selectDefaultSubnational = () => {
    Object.keys(subnationalsGeoJsonRef.current?._layers).forEach((k: any) => {
      if (
        subnationalsGeoJsonRef.current?._layers[k].feature.properties
          .shapeISO === defaultJurisdictionCode ||
        subnationalsGeoJsonRef.current?._layers[k].feature.properties
          .shapeName === defaultJurisdictionName
      )
    ...
    <>
      ...
      <MapContainer
        ...
        whenCreated={(mapInstance: any) => {
          ...
          if (defaultCountry) selectDefaultCountry();
          if (defaultSiteName) selectDefaultSite();
        }}
        center={[20, 30]}
        zoom={3}
      ...
      >
        ...
        {markers.map((marker: any, index: number) => (
          <Marker
            key={`nested-account-map-marker-${index}`}
