...
const NestedAccounts: FunctionComponent<IProps> = (props) => {
  ...
  const countrySelectedHandler = async (countryName: string) => {
    const countryAggrEmissions = aggregatedEmissions?.filter(
      (ae) => ae.facility_country?.toLowerCase() === countryName.toLowerCase()
    );
    const summaryAggrEmissions =
      AggregatedEmissionHelper.GetSummaryAggregatedEmissions(
        countryAggrEmissions
      );
  ...
  const subnationalSelectedHandler = (subnationalName: string) => {
    if (subnationalName) {
      const countryAggrEmissions = aggregatedEmissions?.filter((ae) =>
        ae.facility_jurisdiction
          ?.toLowerCase()
          .includes(subnationalName.toLowerCase())
      );
      const summaryAggrEmissions =
        AggregatedEmissionHelper.GetSummaryAggregatedEmissions(
          countryAggrEmissions
  ...
  const siteSelectedHandler = (siteId: string) => {
    ...
    if (selectedSite) {
      const siteAggrEmissions = aggregatedEmissions?.find(
        (ae) => ae.facility_name === selectedSite.facility_name
      );
      if (siteAggrEmissions) setSummaryEmissions(siteAggrEmissions);
      else {
        setSummaryEmissions({
  ...
  const loadSiteClimateActions = () => {
    ...
    switch (panelType) {
      case PanelType.Country:
        sitesClimateActions = sites.filter(
          (s) => s.facility_country === selectedArea
        );
        break;
      case PanelType.Subnational:
        sitesClimateActions = sites.filter(
          (s) => selectedArea === s.facility_jurisdiction
        );
        break;
      case PanelType.Site:
        sitesClimateActions = sites.filter(
          (s) => s.facility_name === selectedArea
        );
        break;
    }

    const missedLoadedSitesData = sitesClimateActions.filter(
      (s) =>
        s.facility_name &&
        !Object.keys(groupedClimateActions).includes(s.facility_name)
    );

    missedLoadedSitesData.forEach((s) =>
      s.facility_name ? loadClimateActions(s.facility_name) : ""
    );
  ...
  const displayClimateActions = () => {
    ...
    climateActionSites?.forEach((s) => {
      if (
        s.facility_name &&
        Object.keys(groupedClimateActions).length !== 0 &&
        selectedArea &&
        Object.keys(groupedClimateActions).includes(s.facility_name)
      ) {
        climateActionsSummary = climateActionsSummary.concat(
          (groupedClimateActions as any)[s.facility_name]
        );
      }
    });

    return <ClimateActionsPanel climateActions={climateActionsSummary} />;
  };
            ...
            <div className="account-sites__climate-widget-wrapper">
              <EmissionWidget
                ...
                title={`Emission Inventory - ${
                  entityTitle ? entityTitle : selectedArea
                }`}
