...
const AccountPage: FunctionComponent<IProps> = (props) => {
  ...
  useEffect(() => {
    if (user && user.company && user.company.organization_id) {
      if (!pledgesLoaded) loadPledges(user.company.organization_id);

      if (!transfersLoaded) loadTransfers(user.company.organization_id);

      if (!sitesLoaded) loadSites(user.company.organization_id);
...
const mapStateToProps = (state: RootState) => {
  return {
    ...
    aggregatedEmissionsLoaded:
      accountSelectors.getAggregatedEmissionsLoaded(state),
