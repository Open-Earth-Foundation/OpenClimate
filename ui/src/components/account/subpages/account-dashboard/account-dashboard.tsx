...
const AccountDashboard: FunctionComponent<IProps> = (props) => {
  ...
  const [summaryEmissions, setSummaryEmissions] =
    useState<IAggregatedEmission>();
  ...
  useEffect(() => {
    if (aggregatedEmissions?.length == 0) {
      ...
      const totalScopeEmissions =
        scope1EmsTotal + scope2EmsTotal + scope3EmsTotal;
      const totalScopeMitigations =
        scope1MtsTotal + scope2MtsTotal + scope3MtsTotal;
  ...
  return (
    <div className="account-dasboard">
      <SitesMapWidget sites={sites} detailsLink="account/sites" />
