  ...
  const selectFilterHandler = (
    ...
    switch (filterType) {
      ...
      case FilterTypes.SubNational:
        ...
        getOptions(actor_id, type).then((options) => {
      ...
      case FilterTypes.National:
        getOptions(actor_id, "adm1").then((options) => {
    ...
    <div className="review__earth-main">
      ...
      {cards.map((card, index) => (
        <React.Fragment key={`card-fragment-${card.label}`}>
          {actors.length > 1 && (
            <div
              className="review__arrow-forward"
              key={`arrow-forward-${index}`}
            >
              <ArrowForwardIos />
            </div>
          )}
          {card.selectedValue ? (
            ...
            <div
              className={
                actors.length > 1
                  ? "review__card-actor-selected"
                  : "review__card"
              }
              key={`review-card-div-${index}`}
            >
              <LevelCard
                label={card.label}
                ...
                onSelect={(option: DropdownOption) =>
                  onActorSelect(option, card.type, card.label)
                }
                onDeSelect={() => deselectFilterHandler(card.type)}
                options={card.options}
                isCity={index === 2 ? isCity : undefined}
                onButtonSwap={
                  index === 2 ? () => setIsCity(!isCity) : undefined
                }
              />
            </div>
          )}
