  ...
  const selectFilterHandler = (filterType: FilterTypes, option: any) => {
    ...
    if (filterType == FilterTypes.National) {
      fetch(`/api/v1/actor/${actor_id}/parts?type=adm1`)
        ...
        .then((json) => {
          let parts = json.data;
          let options = parts.map((part: any) => {
            return { name: part.name, value: part.actor_id };
          });
          let u = fltr.slice();
      ...
      fetch(`/api/v1/actor/${actor_id}/parts?type=${type}`)
        ...
        .then((json) => {
          ...
          u[3] = {
            ...u[3],
            selectedValue: null,
            options: u[3].hidden ? [] : options,
          };
          u[4] = {
            ...u[4],
            selectedValue: null,
            options: u[4].hidden ? [] : options,
          };
          setFltr(u);
        });
    }
  ...
  const filtersHtml = fltr?.map((f: any, i: number) => {
    ...
    return f?.isRadio ? (
      <div className="review__filter review__radio-filters" key={i}>
        <FormControl>
          <FormLabel className="review__filter-form-label">
            {"Entity Type"}
          </FormLabel>
          <RadioGroup
          ...
          >
            <FormControlLabel
              ...
              value="City"
              control={<Radio size="small" classes={{ root: classes.root }} />}
              label="City"
            />
            <FormControlLabel
              classes={{ label: classes.label }}
      ...
      <div className="review__filter" key={i}>
        <Dropdown
          ...
          onSelect={(option: DropdownOption) =>
            selectFilterHandler(f.type, option)
          }
          disabled={disabled}
