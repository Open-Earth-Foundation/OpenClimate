...
const ContextBarData: FunctionComponent<Props> = (props) => {
  ...
  if (entity.title === "Earth") {
    ...
    let items = entity.pledges?.map((ed, index) => (
      <ContextBarItem
        key={`context-bar-item-${index}`}
        title={`${ed.pledge_emission_reduction ?? 0}%`}
        showError={true}
      >
        ...
        {ed.pledge_base_year ? <>relative to {ed.pledge_base_year}</> : ""}
      </ContextBarItem>
    ));
  ...
  return (
    <div className={`context-bar__data`}>
      <div className="context-bar__data_grid">{displayHtml}</div>
