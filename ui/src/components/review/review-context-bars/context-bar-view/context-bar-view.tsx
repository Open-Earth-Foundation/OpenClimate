...
const ContextBarView: FunctionComponent<Props> = (props) => {
  ...
  const icon = entity.flagCode ? (
    <CircleFlag countryCode={entity.flagCode} height={entityHeight} />
  ) : (
    <img src={earthIcon} />
  );
  ...
  return (
    <div className={`context-bar__view`}>
      <div className="context-bar__pic-wrapper">
        <div className="context-bar__pic">{icon}</div>
        <div className="context-bar__title">{entity.title}</div>
      </div>
    </div>
  );
};
